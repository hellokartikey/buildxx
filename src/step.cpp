...
namespace bxx {
step::step(private_tag, std::shared_ptr<ctx> ctx, fs::path exe, arguments args, environment env)
  : m_ctx(ctx)
  , m_exe(exe)
  , m_args(args)
  , m_env(env)
...
std::shared_ptr<step> step::create(
...
std::shared_ptr<step> step::add_env(env::key k, env::value v) { m_env[k] = v; return get(); }
